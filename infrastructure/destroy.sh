...
aws s3 cp s3://polyglotDataNerd-bigdata-utility/terraform/CCPA/$environment/$CURRENTDATE ~/sg-spark-ccpa/infrastructure/main  --recursive --sse --quiet --include "*"

export TF_VAR_awsaccess=$AWS_ACCESS_KEY_ID
export TF_VAR_awssecret=$AWS_SECRET_ACCESS_KEY
export TF_VAR_environment=$environment

cd ~/sg-spark-ccpa/infrastructure/main
terraform init
terraform destroy -auto-approve

#copy tfstate files to s3
aws s3 cp ~/sg-spark-ccpa/infrastructure/main/ s3://polyglotDataNerd-bigdata-utility/terraform/CCPA/environment/$CURRENTDATE/  --recursive --sse --quiet --exclude "*" --include "*terraform.tfstate*"

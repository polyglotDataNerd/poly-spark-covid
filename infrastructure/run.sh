...
GitHash=$(cd ~/sg-spark-ccpa && (git rev-parse --verify HEAD))
EpochTag="$(date +%s)"
...
TaskDef=$(cat <<-END
{
  "family": "sg-spark-ccpa-$environment",
  "networkMode": "awsvpc",
  ...
  "containerDefinitions": [
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "sg-spark-ccpa-$environment",
...
cd ~/sg-spark-ccpa/infrastructure/environment/build
